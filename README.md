# LocalRank — Local SEO Platform

Get found when local customers search. LocalRank crawls a business website, scores its local SEO on a 100-point scale, and produces a prioritized fix list with real recommendations.
...
- **Subscriptions** — Free / Pro ($19/mo) / Agency ($79/mo) with server-side plan enforcement.
...
User clicks Pay (Billing page)
  ...
  -> LocalRank stores a PENDING payment record
  ...
  -> LocalRank polls Paynow's poll URL (signed with the integration key)
...
2. Pick **Pro** or **Agency**, choose a payment method (EcoCash / OneMoney / Visa-Master).
